...
#[test]
fn test_join() {
    let query = "connector.table | join kind=inner (other.table) on field1 == field2";
    let result = parse_unwrap!(query);
    ...
    match &result[1] {
        QueryStep::Join(join, sub_query) => {
            ...
            assert_eq!(join.on.0, field_unwrap!("field1"));
            assert_eq!(join.on.1, field_unwrap!("field2"));
...
fn test_join_types(join_type_str: &str, expected_type: JoinType) {
    let query = format!(
        "connector.table | join kind={} (other.table) on field1 == field2",
        join_type_str
    );
    let result = parse(&query).unwrap();
...
fn test_join_with_partitions() {
    let query =
        "connector.table | join kind=inner hint.partitions=4 (other.table) on field1 == field2";
    let result = parse_unwrap!(query);
