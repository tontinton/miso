...
mod tests {
    ...
    mod compile_filter {
        ...
        #[test]
        fn or_includes_minimum_should_match() {
            let expr = Expr::Or(eq_field("a", lit_int(1)), eq_field("b", lit_int(2)));
            assert_eq!(
                compile_filter_ast(&expr).unwrap(),
                json!({"bool": {
                    "should": [
        ...
        fn and_and_not() {
            ...
            assert_eq!(
                compile_filter_ast(&Expr::And(eq_a.clone(), eq_b)).unwrap(),
                json!({"bool": {"must": [
                    {"term": {"a": {"value": "1"}}},
                    {"term": {"b": {"value": "2"}}},
                ]}})
        ...
        #[test]
        fn has() {
            let expr = Expr::Has(field_expr("msg"), lit_str("error"));
            assert_eq!(
                compile_filter_ast(&expr).unwrap(),
    ...
    mod handle {
        ...
        fn with_filter_accumulates() {
            let h = ElasticsearchHandle::new(None)
                .with_filter(json!({"term": {"a": "1"}}))
                .with_filter(json!({"term": {"b": "2"}}));
            assert_eq!(h.query.len(), 2);
        }

        #[test]
        fn with_count_clears_source() {
            let h = ElasticsearchHandle::new(None)
                .with_project(vec!["a".into(), "b".into()])
                .with_count();
            assert!(h._source.is_none());
            assert!(h.count);
        }

        #[test]
        fn with_limit_preserves_other_state() {
            let h = ElasticsearchHandle::new(Some("ts".into()))
                .with_filter(json!({"term": {"a": "1"}}))
                .with_limit(10);
            assert_eq!(h.size, Some(10));
            assert_eq!(h.query.len(), 1);
