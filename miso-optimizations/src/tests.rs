...
#[test_case(
        vec![S::Project(vec![rename_project("a", "b")])],
...
#[test_case(
        vec![S::Project(vec![rename_project("a", "b"), literal_project("c", int_val(50))])],
...
#[test_case(
        vec![S::Extend(vec![rename_project("a", "b")])],
        S::Filter(Expr::Eq(Box::new(Expr::Field(field("a"))), Box::new(Expr::Literal(string_val("test"))))),
        vec![
            S::Filter(Expr::Eq(Box::new(Expr::Field(field("b"))), Box::new(Expr::Literal(string_val("test"))))),
...
fn test_project_propagation_through_filter(project_step: Vec<S>, filter_step: S, expected: Vec<S>) {
    let mut input = project_step;
    input.push(filter_step);
...
fn test_project_propagation_through_sort(project_step: S, sort_step: S, expected: Vec<S>) {
    check_default(vec![project_step, sort_step], expected);
}
...
#[test_case(
        vec![literal_project("c", int_val(100))],
        S::TopN(vec![sort_asc(field("c"))], 3),
        vec![S::Project(vec![literal_project("c", int_val(100))])]
        ; "literal topn removed"
    )]
fn test_project_propagation_through_topn(
    project_fields: Vec<ProjectField>,
    topn_step: S,
    expected: Vec<S>,
) {
    check_default(vec![S::Project(project_fields), topn_step], expected);
}
...
fn test_project_propagation_through_limit(project_step: S, limit_step: S, expected: Vec<S>) {
    check_default(vec![project_step, limit_step], expected);
}
