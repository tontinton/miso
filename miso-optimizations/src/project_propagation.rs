...
pub struct ProjectPropagationWithoutSummarize;

/// Example with summarize:
///   project a = b, c = 50 | summarize d = sum(a) by c
///  ->
///   summarize d = sum(b) | extend c = 50
pub struct ProjectPropagationWithSummarize;

impl Optimization for ProjectPropagationWithoutSummarize {
    fn pattern(&self) -> Pattern {
        pattern!([Project Extend] ([Filter Sort TopN Limit]+))
    }
...
impl Optimization for ProjectPropagationWithSummarize {
    fn pattern(&self) -> Pattern {
        pattern!([Project Extend] ([Filter Sort TopN Limit]*?) [Summarize MuxSummarize])
    }
...
fn apply(
        ...
        for step in steps[middle_start..middle_end].iter().cloned() {
            let new = match step {
                ...
                WorkflowStep::Sort(sorts) => {
                    if let Some(sorts) = rewrite_sorts(sorts, &renames, &literals) {
                        WorkflowStep::Sort(sorts)
                    } else {
                        continue;
                    }
    ...
    let (WorkflowStep::Summarize(sum) | WorkflowStep::MuxSummarize(sum)) = tail else {
        unreachable!("not in summarize pattern");
    };

    let (new_sum, post_project_fields) = rewrite_summarize(sum.clone(), &renames, &literals)?;
...
fn rewrite_sorts(
    ...
    (!sorts.is_empty()).then_some(sorts)
}
