...
fn to_workflow(
    ...
    for step in &query_steps {
        match step {
            QueryStep::Filter(ast) if connector.can_filter(ast) => {
    ...
    let number_of_pushdown_steps = pushdown_filters.len();
    if !pushdown_filters.is_empty() {
        let limit = if number_of_pushdown_steps == query_steps.len() {
            limit
        } else {
            None
        };
        steps.push(WorkflowStep::Scan {
            ...
            // TODO: don't add AND like this, do it before predicate pushdown.
            pushdown: Some(FilterAst::And(pushdown_filters)), 
            limit,
        });
    }
