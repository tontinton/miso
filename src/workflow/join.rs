...
type JoinMap = BTreeMap<KeyString, Vec<Log>>;
...
fn merge_two_logs(left: Log, right: Log) -> Log {
    let mut merged = left;
...
fn two_sided_join(keys: Vec<KeyString>, mut left: JoinMap, mut right: JoinMap) -> LogTryStream {
    Box::pin(try_stream! {
        for key in keys {
            match (left.remove(&key), right.remove(&key)) {
                (Some(left_logs), Some(right_logs)) => {
                    for (left_log, right_log) in iproduct!(left_logs, right_logs) {
                        yield merge_two_logs(left_log, right_log);
                    }
                }
                (Some(left_logs), None) => {
    ...
    loop {
        tokio::select! {
            Some(log) = left_stream.next() => {
                if log.contains_key(&left_key) {
                    left.entry(left_key.clone())
                        .or_insert_with(Vec::new)
                        .push(log);
                }
            },
            Some(log) = right_stream.next() => {
                if log.contains_key(&right_key) {
                    right.entry(right_key.clone())
                        .or_insert_with(Vec::new)
                        .push(log);
                }
            },
